...
ScreenScheduler::ScreenScheduler(DisplayDriver& display) : display(display) {}

void ScreenScheduler::configure(bool nextEnabled, const String& offTime, const String& onTime,
                                uint64_t timestampMs, int16_t nextTimezoneOffset) {
  enabled = nextEnabled;
  offMinute = parseMinute(offTime, offMinute);
  onMinute = parseMinute(onTime, onMinute);
  timezoneOffsetMinutes = nextTimezoneOffset;
...
void ScreenScheduler::tick() {
  ...
  Serial.printf("schedule: backlight=%s\n", screenOn ? "on" : "off");
}
